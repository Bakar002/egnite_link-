      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8 text-center py-20">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-6 leading-tight tracking-tight max-w-5xl">
          <span className="inline-block bg-gradient-to-r from-white via-blue-100 to-cyan-200 bg-clip-text text-transparent">
            Systems Are Not Smart Until They Learn.
          </span>
        </h1>
        ...
        <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl w-full">
          {[
            { icon: '🧠', title: 'AI-Powered', desc: 'Machine Learning Integration' },
            { icon: '⚡', title: 'Real-Time', desc: 'Instant Processing' },
            { icon: '🔒', title: 'Secure', desc: 'Enterprise Grade Security' }
          ].map((item, idx) => (
            <div 
              key={idx}
              className="relative group bg-white/5 backdrop-blur-sm border border-cyan-500/20 rounded-xl p-6 hover:bg-white/10 hover:border-cyan-400/40 transition-all duration-300"
            >
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl blur-xl"></div>
              </div>
            </div>
          ))}
        </div>
